...
def try_http(debug):
    """Fast path intended for a normal residential/self-hosted network."""
    ...
    try:
        ...
        for slug in ("slots", "recommended-slots"):
            r = s.post(API_URL, headers=headers, json={"query": QUERY, "variables": vars_for(slug)}, timeout=30)
            try:
                body = r.json()
            except Exception:
                body = None
            rows = normalize(body) if body else []
            debug.setdefault("http_attempts", []).append({"slug": slug, "status": r.status_code, "count": len(rows)})
...
def try_browser(debug):
    """Fallback: reuse the exact frontend query inside Stake's own browser session."""
    observed = []
    best_rows = []
        ...
        def on_request(req):
            ...
            try:
                ...
                try:
                    ...
                    return
            if isinstance(payload, dict) and "SlugKuratorGroup" in (payload.get("query") or ""):
                observed.append({
        ...
        try:
            ...
            for obs in reversed(observed):
                ...
                if v.get("slug") not in {"slots", "recommended-slots"}:
                    continue
                v.update({"limit": 50, "offset": 0, "sort": "popular", "showGames": True, "isActivePlayersFeatureFlagOn": True})
            ...
            def on_response(resp):
                ...
                try:
                    ...
                    if callable(payload):
                        payload = payload()
                    if not isinstance(payload, dict) or "SlugKuratorGroup" not in (payload.get("query") or ""):
                        return
                    body = resp.json()
                    rows = normalize(body)
            ...
            for _ in range(12):
                if len(collected) >= 50:
                    break
                clicked = False
                for label in ("載入更多", "加载更多", "Load More", "顯示更多", "Show More"):
                    try:
...
def main():
    ...
    if len(rows) != 50:
        ...
        print(json.dumps({"status": "failed", "count": len(rows), "debug": debug}, ensure_ascii=False))
        sys.exit(2)

    OUT.parent.mkdir(parents=True, exist_ok=True)
    OUT.write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps({"status": "ok", "count": 50, "capturedAtTaipei": captured, "source": source}, ensure_ascii=False))
